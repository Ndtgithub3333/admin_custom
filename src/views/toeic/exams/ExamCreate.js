...
const ExamCreate = () => {
  ...
  const handleValidate = () => {
    ...
    if (Object.keys(newErrors).length > 0 || !validExcel) {
      setIsValid(false)
      setValidationMessage(
        'Vui lòng điền tất cả các trường bắt buộc và đảm bảo file Excel không có lỗi.',
      )
      return
    }
    ...
    setIsValid(true)
    setValidationMessage('Xác thực thành công!')
  }

  const handleSubmit = (e) => {
    e.preventDefault()
    if (!isValid) {
      setValidationMessage('Vui lòng kiểm tra lại file Excel và các trường khác trước khi lưu.')
      return
    }
    
    // Log tất cả dữ liệu ra console
    console.log({
      examData,
      type,
      files,
  ...
  const resetForm = () => {
    setExamData({ title: '', difficulty: 'intermediate', status: 'Draft' })
            ...
            <CForm onSubmit={handleSubmit}>
              ...
              <CRow className="mb-3">
                <CCol className='mb-1' md={12}>
                  <label className='me-1' htmlFor="">Image</label>
                  <input
                    type="file"
                    name="images"
                    multiple
                ...
                <CCol className='mb-1' md={12}>
                  <label className='me-1' htmlFor="">Audio</label>
                  <input
                    type="file"
                    name="audioFiles"
                    multiple
                ...
                <CCol className='mb-1' md={12}>
                  <label className='me-2' htmlFor="">Excel</label>
              ...
              <div style={{ marginTop: '20px' }}>
                ...
                {errorCells.length > 0 && (
                  <div style={{ color: 'red', marginTop: '10px' }}>
                    <h6>Các lỗi phát hiện:</h6>
                    <ul>
                      {errorCells.map((error, index) => (
                        <li key={index}>
                          Lỗi ở hàng {error.row}, cột {error.col + 1}
                        </li>
                      ))}
                    </ul>
                  </div>
